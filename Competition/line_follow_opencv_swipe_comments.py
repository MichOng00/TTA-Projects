...
#   to change if the model is relabeled/retrained with a different name.

UNRIPE_CONF_THRESHOLD = 0.85  # ignore low-confidence detections
# ^ How sure the model needs to be before the robot reacts (0 = not sure at
#   all, 1 = 100% certain). Raising this (e.g. to 0.9) helps if the robot
#   is stopping for things that aren't really unripe cubes. Lowering it
#   (e.g. to 0.6) helps if it's missing real cubes.

DETECT_EVERY_N_FRAMES = 1  # run YOLO every Nth frame; raise this if it's slowing the loop down
# ^ How often the robot checks for cubes. 1 = check every camera frame
#   (most responsive, but uses the most computer power). If the robot
#   feels laggy, 2 or 3 makes it only check every 2nd/3rd frame.

UNRIPE_Y_THRESHOLD = 150
# ^ How close a cube needs to be (in pixels down the camera image) before
#   the robot reacts to it. A bigger number means the cube must be
#   closer/lower in the frame before the robot stops. A smaller number
#   means the robot reacts to cubes further away too.
...
def main():
    ...
    while True:
        ...
        # (UNRIPE_CONF_THRESHOLD and UNRIPE_Y_THRESHOLD near the top of
        # the file control how sensitive this is.)
        # -------------------------------------------------------------
