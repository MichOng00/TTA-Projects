...
Controls:
    ...
    DRAG to matching pedestal = snap orb in place
...
WINDOW_NAME = "Mystic Shelf — Session 06"
TARGET_W = 1280
TARGET_H = 720
FPS = 60
...
def ensure_model():
    if not os.path.exists(MODEL_PATH):
        print("[Session 06] Downloading hand_landmarker.task (~8 MB) ...")
        urllib.request.urlretrieve(MODEL_URL, MODEL_PATH)
        print("[Session 06] Model saved to", MODEL_PATH)
...
def run():
    ...
    if not cap.isOpened():
        print("[Session 06] ERROR: Unable to open the camera.")
        return
    ...
    while running:
        ret, frame = cap.read()
        if not ret:
            print("[Session 06] ERROR: Camera frame read failed.")
            break
