...
Concepts:
    ...
    - Score tracking
    - State management (grabbed, placed)
...
# Constants
WINDOW_NAME = "Session 05: Orb Interaction & Snapping"
TARGET_W = 1280
TARGET_H = 720
FPS = 60
...
def ensure_model():
    if not os.path.exists(MODEL_PATH):
        print("[Session 05] Downloading hand_landmarker.task (~8 MB) ...")
        urllib.request.urlretrieve(MODEL_URL, MODEL_PATH)
        print("[Session 05] Model saved to", MODEL_PATH)
...
class Particle:
    """Visual effect particle that floats and fades out."""
...
class Pedestal:
    """Target platform for orb placement."""
    def __init__(self, name, color, pos):
        self.name = name
        self.color = color
        self.pos = pos
...
def run():
    ...
    if not cap.isOpened():
        print("[Session 05] ERROR: Unable to open the camera.")
        return
    ...
    while running:
        ...
        if is_pinching and pinch_pos:
            ...
            # Release orb
            if grabbed_orb:
                # Try to snap to matching pedestal
        ...
        draw_text_centered(screen, "SESSION 05: Orb Interaction & Snapping", (W // 2, 25), font_large, (200, 160, 255))
        draw_text_centered(screen, f"PLACED  {score} / {NUM_ORBS}", (W // 4, 25), font_medium, (160, 220, 255))
        draw_text_centered(screen, "PINCH to grab and drag | Match colors | Q = quit",
