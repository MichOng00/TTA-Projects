...
class Orb:
    ...
    def draw(self, surface):
        ...
        draw_text(surface, self.name.upper(), (cx, cy), pygame.font.Font(None, 18))
...
class Pedestal:
    ...
    def draw(self, surface, dt):
        cx, cy = self.pos
        pygame.draw.circle(surface, self.color, (cx, cy), PEDESTAL_RADIUS + 6, 2)
        pygame.draw.circle(surface, self.color, (cx, cy), PEDESTAL_RADIUS - 4, 2)
        draw_text(surface, self.name.upper(), (cx, cy + PEDESTAL_RADIUS + 20), pygame.font.Font(None, 18))
        if self.filled:
...
def make_game(w, h):
    ...
    pedestals = [Pedestal(n, COLORS[n], (int(xs[i]), pedestal_y)) for i, n in enumerate(COLOR_NAMES)]
    orbs = [Orb(n, COLORS[n], 
