"""
Robot control script for locating, picking up, and returning AprilTag-marked objects using a mecanum wheel robot with mechanical arm. 
Integrates OpenCV for vision processing and the UGOT API for movement, arm manipulation, and onboard display feedback.

Note: only guaranteed to work with Python >= 3.11 and will not work with 3.6; other versions may work but are untested.
"""
...
def go_there():
    ...
    got.mecanum_move_speed_times(0, 30, 50, 1) # 0 is forward, 1 is backward
    got.mecanum_turn_speed_times(3, 45, 90, 2) # 3 is left, 2 is right
...
def pick_up():
    ...
    got.mechanical_joint_control(0, -20, -40, 500) # joint 1, 2, 3, duration
    time.sleep(1)
    ...
    if got.get_apriltag_total_info(): # empty list if no tags
        # Tag still visible -> failed grasp
        got.screen_display_background(3)  # red screen background
        got.mechanical_clamp_release()    # drop object
        return False
    else:
        # Tag no longer visible -> successful grasp
        got.screen_display_background(6)  # green screen background
        return True
