...
def get_quickdraw_class_names():
    ...
    # list comprehension - shorter than creating empty list + for loop
    return classes


def download_quickdraw_dataset(root="../QuickDraw", limit: Optional[int] = None, class_names: List[str]=[]):
...
class QuickDrawDataset(torch.utils.data.Dataset):
    def __init__(self, root, max_items_per_class = 5000, class_limit = None, class_names = []):
        super().__init__()
        self.root = root
    ...
    def __getitem__(self, index):
        x = (self.X[index] / 255.0).astype(np.float32).reshape(1, 28, 28)
        y = self.Y[index]
