...
class Dinosaur:
    ...
    def update(self):
        if self.dino_jump:
            ...
            self.duck()
            if self.rect.y > self.Y_POS_DUCK:
...
def eval_genomes(genomes, config):
    ...
    while run:
        ...
        if len(obstacles) == 0:
            ...
            if choice == 0:
                obstacles.append(SmallCactus(SMALL_CACTUS, random.randint(0, 1)))
            elif choice == 1:
                obstacles.append(LargeCactus(LARGE_CACTUS, random.randint(0, 1)))
            else:
        ...
        for i, dinosaur in enumerate(dinosaurs):
            ...
            output = nets[i].activate((
                ...
                # distance((dinosaur.rect.x, dinosaur.rect.y),
                #         obstacle.rect.midbottom),
                # (dinosaur.rect.centerx - obstacle.rect.centerx), 
                game_speed,
                # obstacle.rect.centery
                isinstance(obstacle, Pterodactyl)
            ))
