...
TRACK = pygame.image.load("Assets/not_car/track_shortcut.png")
...
LAP_FITNESS_BASE = 1000    # baseline so ANY finished lap beats ANY unfinished attempt
DEATH_PENALTY = 20         # fitness knocked off a car's score when it crashes into the grass

# Cars now control their own speed (not just steering), so faster/slower
# driving styles actually produce different lap times. MIN_SPEED keeps
# every car moving (matches the old "always drive forward" behavior);
# MAX_SPEED caps how fast the network can push it.
MIN_SPEED = 3.0
MAX_SPEED = 14.0
...
class Car(pygame.sprite.Sprite):
    def __init__(self, color=None):
        ...
        self.direction = 0
        self.gear = 1
        self.speed = MIN_SPEED               # current forward speed, set each frame by the network's throttle output
    ...
    def apply_controls(self, throttle, steering):
        ...
        # Map throttle onto a speed between MIN_SPEED and MAX_SPEED.
        # Clamped to [0, 1] first in case the network's output range
        # overshoots slightly (e.g. with a sigmoid activation this is a
        # no-op; adjust the clamp if your config uses a different
        # activation with a different output range, like tanh's [-1, 1]).
