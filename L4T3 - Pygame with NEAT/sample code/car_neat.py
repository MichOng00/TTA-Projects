...
def eval_genomes(genomes, config):
    ...
    fitness_threshold = 5000           # optional early stop if fitness reached
    distance_threshold = 100           # if car hasn't moved this far after 1000 steps, kill it
    ...
    while run and len(cars) > 0 and steps < max_steps:
        ...
        for i, car in enumerate(cars):
            if car.alive:
                ge[i].fitness += 1
                # # Optional: kill cars that are stuck (not progressing)
                # if steps > 1000 and car.distance_from_start() < distance_threshold:
                #     car.alive = False
                # Also kill if fitness exceeds threshold (optional early stop)
                if ge[i].fitness >= fitness_threshold:
                    # break out of the generation loop (all cars die)
                    car.alive = False
